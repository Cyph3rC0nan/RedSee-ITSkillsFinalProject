...
"""
Deterministic sandboxed recon runners — httpx (HTTP fingerprinting) and tlsx
...
httpx and tlsx findings are BROADER than the frozen schemas.py Finding enum
(same reasoning as nuclei — see DECISIONS.md D-017), so ReconObservation is a
LOCAL dataclass, never mapped to schemas.Finding. Surfacing into
...
if __name__ == "__main__":
    ...
    print(f"running httpx + tlsx recon against {target!r} ...")
    httpx_obs = run_httpx([target], scope_config=scope)
    tlsx_obs = run_tlsx([target], scope_config=scope)

    for label, obs_list in (("httpx", httpx_obs), ("tlsx", tlsx_obs)):
